...
# ========= 路径与输出 =========
# 兼容 /data/gsod/extracted/<year>/*.csv 与 /data/gsod/<year>/*.csv
GSOD_BASE_CANDIDATES = ["/data/gsod/extracted", "/data/gsod"]

INDEX_DIR = os.path.expanduser(
    "~/yangtze-1998-wrfhydro-rri/docs/figs/gsod_index"
)
OUT_DIR   = os.path.expanduser(
    "~/yangtze-1998-wrfhydro-rri/docs/figs/gsod_maps"
)
...
def load_points_from_index(y, bbox, china_codes):
    """优先用索引（包含 country 列时更快更稳）。
       返回：(bbox内所有点, bbox∩中国点) —— 都是 [(lon,lat), ...] 列表。"""
    f = os.path.join(INDEX_DIR, f"gsod_index_{y}.csv")
    pts_all, pts_china = [], []
    if os.path.exists(f):
        df = pd.read_csv(f)
        # 兼容没有 country 列的旧索引
    ...
    for fp in glob.glob(os.path.join(ydir, "*.csv")):
        try:
            t = pd.read_csv(fp, nrows=1, usecols=["LATITUDE","LONGITUDE"])
            if t.empty: continue
            lat = float(t.iloc[0]["LATITUDE"]); lon = float(t.iloc[0]["LONGITUDE"])
...
def main():
    ...
    ap.add_argument("--china-codes", default="CHN,CH",
                    help="与索引country列匹配的中国国家码，逗号分隔；默认兼容 CHN 与 CH")
    args = ap.parse_args()
    ...
    # 输出统计
    count_all_txt   = os.path.join(OUT_DIR, "yearly_counts_gsod_bbox.txt")
    count_china_txt = os.path.join(OUT_DIR, "yearly_counts_gsod_bbox_china.txt")
