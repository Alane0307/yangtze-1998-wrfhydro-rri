...
# ===== 路径与窗口设置（保持你原有输出路径命名风格，只把 gsod → isd） =====
ISD_BASE = os.path.expanduser("/data/isd/extracted")  # 备用回退扫描目录
INDEX_DIR = os.path.expanduser("/data/isd/index")     # isd_build_index.py 默认索引输出目录
OUT_DIR   = os.path.expanduser("~/yangtze-1998-wrfhydro-rri/docs/figs/isd_comparison")

# 外层嵌套窗口
BBOX = dict(lon_min=85.5, lon_max=130.0, lat_min=18.0, lat_max=40.0)

# —— 叠加用地理图层（路径与 gsod 版一致）——
...
# ===== 读取“全部”或“中国子集”的年站点 =====
def load_points_for_year(y: int, bbox: dict, china_only: bool, china_codes=("CHN","CH","CN")):
    # 优先用索引（更快、可用 country 列）
    index_csv = os.path.join(INDEX_DIR, f"isd_index_{y}.csv")
    ...
    # 无索引 → 退化：扫该年目录读第一条记录拿经纬度（容错 LAT/LON 命名）
    year_dir = os.path.join(ISD_BASE, str(y))
...
# ===== 面板绘制（两张：outer_nest / outer_nest_china_only） =====
def draw_panel(points_list, title_setup, out_png):
    ...
    handles, labels = [], []
    handles.append(Line2D([], [], marker='o', color='none', markerfacecolor=STATION_FC, markersize=8)); labels.append("ISD Station")
    if BASE.main is not None and not BASE.main.empty:
    ...
    # 标题仅将 GSOD → ISD
    fig.suptitle(f"ISD Stations  |  setup: {title_setup}", y=0.985, fontsize=13)
...
def main():
    parser = argparse.ArgumentParser(description="ISD 2×2 yearly comparison (1931/1935/1954/1998)")
    parser.add_argument("--outdir", default=OUT_DIR, help="输出目录（默认仓库 docs/figs/isd_comparison）")
    parser.add_argument("--china-codes", default="CHN,CH,CN", help="country 列中国代码（逗号分隔）")
    ...
    # 输出两个文件（命名与 gsod 版一致风格，仅前缀改 isd）
    out_png_all   = os.path.join(args.outdir, "isd_stations_comparison_2x2_equal.png")
    out_png_china = os.path.join(args.outdir, "isd_stations_comparison_2x2_equal_china.png")
